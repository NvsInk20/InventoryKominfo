...
<!-- Dropdown Container -->
<form method="GET" action="{{ route('Pj.User') }}">
        ...
        <div class="dropdown">
            <div @click.away="openSort2 = false" class="relative" x-data="{ openSort2: false, sortType2: '{{ request('sort_type2', 'Bidang') }}' }">
                ...
                <div x-show="openSort2" x-transition class="dropdown-content">
                    <div class="px-2 pt-2 pb-2 text-black rounded-md shadow-lg bg-white">
                        <div class="flex flex-col">
                            <a @click.prevent="if (sortType2 !== 'Bidang 1') { sortType2='Bidang 1'; openSort2=false; $nextTick(() => $el.closest('form').submit()); }"
                                class="flex flex-row items-start rounded-lg bg-transparent p-2 hover:bg-gray-200"
                                href="#">
                                <p class="font-semibold">Bidang 1</p>
                            </a>
                            <a @click.prevent="if (sortType2 !== 'Bidang 2') { sortType2='Bidang 2'; openSort2=false; $nextTick(() => $el.closest('form').submit()); }"
                                class="flex flex-row items-start rounded-lg bg-transparent p-2 hover:bg-gray-200"
                                href="#">
                                <p class="font-semibold">Bidang 2</p>
                            </a>
                            <a @click.prevent="if (sortType2 !== 'Bidang 3') { sortType2='Bidang 3'; openSort2=false; $nextTick(() => $el.closest('form').submit()); }"
                                class="flex flex-row items-start rounded-lg bg-transparent p-2 hover:bg-gray-200"
                                href="#">
